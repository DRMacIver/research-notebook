...
then that distribution must take the form \(P(X = x) = c \exp\left( \sum \lambda_i f_i(x)  \right)\) for appropriately chosen constants \(c, \lamba_i\) to achieve the desired constraints and make the probabilities add up to one.
